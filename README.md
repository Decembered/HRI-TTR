...
manifest. Bone-Seed is not part of this pipeline. The only source datasets are
the frozen `humanl3d` (HumanML3D) and `interx` trees under
`HRI-Datasets/filtered`. First build their source-reference manifest:

```sh
python scripts/prepare_same_motion_dataset.py \
  --filtered-root /data/autovla/projects/HRI-Datasets/filtered \
  --output /data/users/autovla/datasets/hri_ttr_same_motion_v2
```

Then build and verify the aligned corpus on the training server:

```sh
hri-ttr data prepare-same-motion \
  --manifest /data/users/autovla/datasets/hri_ttr_same_motion_v2/manifest/same_motion.jsonl \
  --output /data/users/autovla/datasets/hri_ttr_same_motion_20hz_v2 \
  --g1-mjcf /data/users/autovla/sonic_repro/gmr/assets/unitree_g1/g1_mocap_29dof.xml \
  --target-fps 20 \
  --workers 16

hri-ttr data audit-corpus \
  --corpus /data/users/autovla/datasets/hri_ttr_same_motion_20hz_v2
