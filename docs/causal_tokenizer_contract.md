...
- Human 和 G1 的 motion feature 各自按第 0 帧 canonicalize；额外的
  `human_space/g1_space` 都保存 InteractionWorld 下的 world `[x,z,yaw]`，不跟随
  feature 的局部 canonical frame 改写。
- codebook 固定 256 项，合法 ID 为 `[0,255]`。
- padding token 不更新 EMA，不计 perplexity；reconstruction loss 和指标由帧 mask
  排除复制的尾帧。
- DDP 下 EMA 使用所有 rank 的有效 token 聚合统计，并广播统一的 dead-code 候选；
  它不是由 DDP 的参数梯度同步自动完成的。
