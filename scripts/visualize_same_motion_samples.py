...
def choose_rows(path: Path, count: int) -> list[dict[str, Any]]:
    rng = random.Random(20260902)
    buckets: dict[str, list[dict[str, Any]]] = {}
    with path.open(encoding="utf-8") as stream:
        for line in stream:
            row = json.loads(line)
            if 2 <= float(row["duration_sec"]) <= 12:
...
def load_g1(
    ...
    if spec["format"] == "npz":
        with np.load(spec["path"]) as data:
            return np.asarray(data["body_positions"][start:end]), model_edges(model)
    if spec["format"] == "joblib_pickle":
    ...
    item = find_pickle_row(Path(spec["path"]), str(spec["locator"]))["motion"]
    return (
        forward_kinematics(
            model,
...
def human_edges() -> list[tuple[int, int]]:
    return [
        (parent, child)
...
def skeleton_svg(
    ...
    circles = [
        f'<circle cx="{x:.1f}" cy="{y:.1f}" r="2.2"/>' for x, y in projected
    ]
    return f'<g stroke="{color}" fill="{color}" stroke-width="3">{"".join(lines + circles)}</g>'


def render(row: dict[str, Any], model: mujoco.MjModel, output: Path) -> dict[str, Any]:
    human = load_human(row)
    g1, g1_edges = load_g1(row, model)
    steps = 6
    ...
    elements = [
        ...
        f'<text x="40" y="45" font-size="24" font-family="sans-serif">{html.escape(row["sample_id"])}</text>',
        '<text x="40" y="90" font-size="20" fill="#176b87" font-family="sans-serif">Human</text>',
        '<text x="40" y="390" font-size="20" fill="#c04b31" font-family="sans-serif">G1 retarget</text>',
    ]
    for index, x in enumerate(columns):
        elements.append(skeleton_svg(human[human_indexes[index]], human_edges(), x, 340, "#176b87"))
        elements.append(skeleton_svg(g1[g1_indexes[index]], g1_edges, x, 640, "#c04b31"))
        elements.append(f'<text x="{x-18:.1f}" y="675" font-size="14">{index*20}%</text>')
    caption = next((item["text"] for item in row["texts"] if item["kind"] == "single_person"), "")
    elements.append(f'<text x="40" y="68" font-size="14" font-family="sans-serif">{html.escape(caption[:180])}</text>')
    elements.append("</svg>")
...
def main() -> None:
    ...
    report = []
    for row in choose_rows(args.manifest, args.per_dataset):
        name = row["sample_id"].replace(":", "_") + ".svg"
        report.append(render(row, model, args.output / name))
