...
app.command("prepare-same-motion")(prepare_same_motion)
app.command("audit-corpus")(audit_corpus)
app.command("smoke-load")(smoke_load)
...
def prepare(
    ...
    # Features are independently canonicalized by each embodiment.  The
    # auxiliary space tracks deliberately stay in the shared InteractionWorld
    # so Stage 3 can compare world trajectories without undoing either feature
    # transform.
    human_space = human_space_states(joints22)
