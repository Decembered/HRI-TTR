...
def prepare_same_motion(
    ...
    bone_records = (
        row for row in _records(manifest) if row.get("source_dataset") == "Bone-Seed"
    )
    with multiprocessing.Pool(
        workers, initializer=_initialize_worker, initargs=(g1_mjcf,)
    ) as pool:
        for index, result in enumerate(
            pool.imap_unordered(_process_worker, bone_records, chunksize=1), start=1
        ):
            _write_result(writer, result)
            if index % 1000 == 0:
                typer.echo(json.dumps({"processed_bone_seed": index}))
    reader = SameMotionSourceReader()
    kinematics = G1Kinematics.from_mjcf(g1_mjcf)
    non_bone = (
        row for row in _records(manifest) if row.get("source_dataset") != "Bone-Seed"
    )
    for index, record in enumerate(non_bone, start=1):
        try:
            writer.add(record, prepare_pair(reader.load(record), kinematics))
        except (OSError, RuntimeError, TypeError, ValueError) as error:
            writer.quarantine(record, error)
        if index % 1000 == 0:
            typer.echo(json.dumps({"processed_other": index}))
