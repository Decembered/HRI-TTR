...
class _ShardBuffer:
    ...
    human_space: list[npt.NDArray[np.float32]] = field(default_factory=list)
    g1_space: list[npt.NDArray[np.float32]] = field(default_factory=list)
...
class CorpusWriter:
    ...
    def add(self, record: Json, pair: PreparedPair) -> None:
        ...
        buffer.human_space.append(pair.human_space.astype(np.float32, copy=False))
        buffer.g1_space.append(pair.g1_space.astype(np.float32, copy=False))
    ...
    def _flush(self, split: str) -> None:
        ...
        human_space = np.concatenate(buffer.human_space, axis=0).astype(
            np.float32, copy=False
        )
        g1_space = np.concatenate(buffer.g1_space, axis=0).astype(
            np.float32, copy=False
        )
        offsets = np.asarray(
        ...
        np.save(destination / "human_space.npy", human_space)
        np.save(destination / "g1_space.npy", g1_space)
    ...
    def _write_normalizers(self) -> None:
        ...
        human = HumanFeatureNormalizer.create(
            ...
            source_dataset="filtered_humanml3d_interx_20hz_v3:train",
            fps=TARGET_FPS,
        )
        ...
        g1_std[contact] = 1.0
        G1FeatureNormalizer(
            g1_mean, g1_std, "filtered_humanml3d_interx_20hz_v3:train", TARGET_FPS
