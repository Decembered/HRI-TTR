...
from hri_ttr.representations.g1.episode import FORWARD_EPSILON, EpisodeFrame
...
class G1RepresentationReason(StrEnum):
    ...
    SPACE = "root space input must be finite [T,3] and [T,4]"
    FORWARD = "horizontal body-forward direction cannot be vertical"
...
def g1_space_states(
    ...
    """Return the per-frame world planar trajectory ``[x_m, z_m, yaw_rad]``.

    The yaw follows the Human space convention (``atan2(forward_z, forward_x)``
    of the horizontal body-forward axis) so G1 and Human space trajectories
    stay directly comparable once placed in one shared world frame.
    """
    positions = np.asarray(root_pos_interaction_m, dtype=np.float64)
    rotations = np.asarray(root_rot_interaction_xyzw, dtype=np.float64)
    if (
        positions.ndim != FEATURE_MATRIX_NDIM
        or rotations.ndim != FEATURE_MATRIX_NDIM
        or positions.shape[0] != rotations.shape[0]
        or positions.shape[1:] != (3,)
        or rotations.shape[1:] != (4,)
    ):
        raise G1RepresentationError(G1RepresentationReason.SPACE)
    if not (np.isfinite(positions).all() and np.isfinite(rotations).all()):
        raise G1RepresentationError(G1RepresentationReason.SPACE)
    forward_world = xyzw_to_matrix(rotations)[:, :, 0].copy()
    forward_world[:, 1] = 0.0
    horizontal = np.sqrt(np.sum(forward_world**2, axis=1))
    if np.any(horizontal <= FORWARD_EPSILON):
        raise G1RepresentationError(G1RepresentationReason.FORWARD)
    forward_world /= horizontal[:, np.newaxis]
    space = np.empty((len(positions), 3), dtype=np.float64)
    space[:, 0] = positions[:, 0]
    space[:, 1] = positions[:, 2]
    space[:, 2] = np.arctan2(forward_world[:, 2], forward_world[:, 0])
    return space
